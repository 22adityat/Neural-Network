...
// inverted format with a positive bmpInfoHeader_biHeight value.
   ...
   public static void main(String[] args) throws IOException
   {
      ...
      FileWriter fw = new FileWriter("//Users//adityatagore//Desktop//NeuralNets//inputdata.txt");
   ...
   // The color table
         ...
         if (args.length > 0)
            ...
            inFileName = "//Users//adityatagore//Desktop//img//img" + iter + ".bmp";

         if (args.length > 1)
            outFileName = args[1];
         else
            outFileName = "//Users//adityatagore//Desktop//img//test" + iter + ".bmp";
   ...
   // System.out.printf("DEBUG: Color Table = %d, %06X\n", i, colorPallet[i]);
         ...
         */
         ...
         for (i = bmpInfoHeader_biHeight - 1; i >= 0; --i)    // write over the rows (in the usual inverted format)
         {
            ...
            {
               imageArray[i][j] = 255 - (dibdumper.colorToGrayscale(imageArray[i][j]) & 0xFF);
               //System.out.print(imageArray[i][j] + " ");
               sumim+=i*imageArray[i][j];
               sumjm+=j*imageArray[i][j];
               sumM+=imageArray[i][j];
            }
            //System.out.println();
         } // for (i = bmpInfoHeader_biHeight - 1; i >= 0; --i)
         //int centeri = 2000;
         //int centerj = 2500;
         int centeri = (int) (sumim/sumM);
         int centerj = (int) (sumjm/sumM);
         System.out.println(centeri + " " + centerj);
         int cropHeight = 55;
         int cropWidth = 55;
         ...
         for (i = cropHeight - 1; i >= 0; i--)
         {
            ...
            {
               newImageArray[i][j] = imageArray[centeri + i - cropHeight/2][centerj + j - cropWidth/2];
               pel = newImageArray[i][j];
               pel = dibdumper.pelToRGB(pel).blue;
               fw.write((((double)pel / 256.0) + 0.02)+ "\n");
            }
         }
   ...
   // there is no color table for this true color image, so write out the pels
            ...
            for (i = bmpInfoHeader_biHeight - 1; i >= 0; --i)    // write over the rows (in the usual inverted format)
            {
               for (j = 0; j < bmpInfoHeader_biWidth; ++j) // and the columns
               {
                  ...
                  rgbQuad_rgbGreen = (pel) & 0x00FF;
                  rgbQuad_rgbRed = (pel) & 0x00FF;
